    ...
    if (detectar_mobile() == false) {
        if (i != text.length) {
            var interval = setInterval(function () {
                if (i != text.length) {
                    homeText.innerHTML = `${homeText.textContent.replace("|", "") + text[i]}|`
                    i++
                } else {
            ...
            setInterval(() => {
                if (homeText.textContent.includes("|")) {
                    homeText.textContent = homeText.textContent.replace("|", "")
                } else {
                    homeText.textContent = `${homeText.textContent}|`
                }
            }, 400);
        }
    } else {
