...
window.addEventListener("load", async() => {
    if (detectar_mobile() != true) {
        let followImg1 = document.getElementById("followImg1")
        let followImg2 = document.getElementById("followImg2")
        let followImg3 = document.getElementById("followImg3")
        const docRef = doc(db, "AllData", "Data");
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
            followImg1.src = `${docSnap.data().Show1}`
            followImg2.src = `${docSnap.data().Show2}`
            followImg3.src = `${docSnap.data().Show3}`
        }
    }
})